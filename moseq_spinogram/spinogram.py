...
from moseq_spinogram.slice import load_h5_timestamps, prep_slice_data
...
def plot_many_single_syllable(args):
...
def plot_syllable_corpus_separate(args):
...
def plot_clustered_corpus(args):
    ...
    for i, sid in enumerate(tqdm(list(reversed(dendro['ivl'])))):
        ...
        if args.show_pca:
            ...
            s, e = slices[0][0]
            data = pca_h5[pca_path][s:e, 0:10]
            x = np.arange(data.shape[0])
            for i in range(data.shape[1]):
                ax.plot(x, data[:,i], label="PC{}".format(i))
            #ax.legend()
...
def produce_spinograms(syllables, examples, axes, args):
    slice_gen = prep_slice_data(args.model, args.index, sort_labels=args.sort, count=args.count)
    all_data = []
    axes_counter = 0
    plot_map = {}
        ...
        for sid in tqdm(syllables, desc="Syllables"):
            ...
            with tqdm(total=examples, desc="Examples", leave=False, disable=True) as pbar:
                ...
                def queue_item(*call_args):
                    future = pool.submit(partial_produce_spinograms, *call_args)
...
def partial_produce_spinograms(args, sid, ex, ax_id, slice):
    try:
...
def create_spinogram_data(slice, numpts=10):
    data = {
        'data': []
    }
...
def create_spinogram_plot(data, color, ax=None):
    if ax is None:
        fig = plt.figure()
